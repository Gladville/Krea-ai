...
const ActionButton = ({ icon: Icon, text }: { icon: React.ElementType; text: string }) => (
  <button className="flex items-center gap-2 mt-3 px-3 py-1.5 dark:text-white text-black rounded-full text-sm hover:border bg-white hover:border-gray-400 dark:bg-gray-700 dark:hover:bg-neutral-600 transition-colors">
    <Icon size={16} />
    <span>{text}</span>
  </button>
);
...
export default function Home() {
  ...
  const handleGenerate = () => {
    ...
    // Simulate API/image generation
    setTimeout(() => {
      setImage('/assets/generated-sample.jpg'); // replace with real API response later
      setLoading(false);
    }, 3000); // fake delay
  };
    ...
    <div className="flex flex-col min-h-screen bg-white dark:bg-neutral-950 text-black dark:text-white p-4">
      ...
      <header className="w-full flex justify-center py-6">
        <div className="flex flex-col items-center gap-4">
          {/* Show default icon + text if idle */}
          {!loading && !image && (
          ...
          {/* Show placeholder when generating */}
          {loading && (
            <div className="w-72 h-48 bg-gray-200 dark:bg-gray-700 rounded-2xl flex items-center justify-center">
              <p className="text-gray-500 dark:text-gray-300">Preparing your image...</p>
            </div>
          )}

          {/* Show generated image once ready */}
          {image && !loading && (
            <img
              src={image}
          ...
          {/* Loading bar */}
          {loading && (
            <div className="w-72 mt-4 bg-gray-300 dark:bg-gray-600 rounded-full h-2 overflow-hidden">
              <div className="bg-blue-600 h-2 animate-pulse w-1/2"></div>
            </div>
          )}
        </div>
      </header>
      ...
      {/* Main Content */}
      <main className="flex-grow flex flex-col items-center justify-center px-4 pb-14">
          ...
          <div className="flex flex-col md:flex-row dark:bg-gray-800 bg-gray-200/100 rounded-3xl p-4 sm:p-6 shadow-lg gap-4">
            ...
            <div className="flex justify-center md:justify-end">
              <button
                ...
                disabled={loading}
                className="w-full md:w-auto md:mt-40 flex items-center justify-center py-3 px-5 bg-white text-black rounded-xl font-semibold text-lg dark:hover:bg-neutral-200 transition-colors shadow-sm hover:shadow-2xl hover:border-gray-300 hover:border-2 disabled:opacity-70"
              >
                {loading ? (
